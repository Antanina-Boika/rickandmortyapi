...
function fetchCharacters(url) {
  fetch(url)
    ...
    .then((data) => {
      let ul = document.querySelector(".pl");
      ...
      activePage = ul.firstElementChild;
      activePage.classList.add("act");
      fetch(`${url}?page=${1}`)
        .then((response) => response.json())
        .then((data) => {
...
//pagination
document.querySelector(".pl").addEventListener("click", function (event) {
  if (event.target.classList.contains("list_style")) {
    event.preventDefault();
    event.stopPropagation();
    let id = event.target.innerHTML;
    activePage.classList.remove("act");
  ...
  if (
    ...
    fetch(url + id)
      ...
      .then((data) => {
        ...
        document.querySelector(".pl").classList.toggle("display_none");
...
//implementation with infinite loading
